...
Additionally, we don't strive for some kind of 100% code coverage. That's not the goal of writing tests in this project. We try to colocate our tests to explain how the codebase for developers, but also ensure that our function behaves in the way we expect it to.
