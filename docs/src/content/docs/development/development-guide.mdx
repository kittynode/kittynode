...
We like logs. In library functions it's essential, in "binding" layers it is highly encouraged (as the API is homogenous and we should trace out the entry here), and in other layers it is optional / as needed. In the other layers it's best to use your judgement but not litter with too many logs.
...
- Rust: `info!()` and `error!()`
- JavaScript: `info()` and `error()`
- todo: How do we bubble errors to the client (or track them otherwise)? Do we use alert() or console.error, or both? I think it is best to not track **any** user data, if we can avoid it. That means we do rely on alert(). The only downside I can think of, is if it's possible to leak things to the client, but the design of Kittynode should be that it is always local. I think the alert pattern is okay, but let's double check.
